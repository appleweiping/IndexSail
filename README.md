...
Opt-in version 4 stores each posting list's strictly increasing document IDs in a canonical Elias–Fano
bit sequence. It stores frequency and positional gaps as the same base-128 varbytes and preserves v3
checksums and block-max metadata. Decoding checks exact lengths, zero padding, one-bit count, strict order,
and the declared maximum before building the searchable index. This is a persistence codec, not a
